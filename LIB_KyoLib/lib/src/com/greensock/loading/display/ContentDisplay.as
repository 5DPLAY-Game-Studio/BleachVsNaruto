 ...
 */	
	public class ContentDisplay extends Sprite {
		...
		protected function _update():void {
			...
			var nativeBounds:Object = (mc is Loader) ? Loader(mc).contentLoaderInfo : (_loader != null && _loader.hasOwnProperty("getClass")) ? mc.loaderInfo : mc.getBounds(mc);  //for SWFLoaders, use loaderInfo.width/height so that everything is based on the stage size, not the bounding box of the DisplayObjects that happen to be on the stage (which could be much larger or smaller than the swf's stage)
			var m:Matrix = mc.transform.matrix;
			var contentWidth:Number = nativeBounds.width * Math.abs(m.a) + nativeBounds.height * Math.abs(m.b)
			var contentHeight:Number = nativeBounds.width * Math.abs(m.c) + nativeBounds.height * Math.abs(m.d);
