...
//				if(getUrlBack != null)getUrlBack();
		...
		public static function addJSCallBack(functionName:String , closure:Function , jsReady:String = null , debugTxt:TextField = null):void{
			if(jsReady == null){
				...
				timer.addEventListener(TimerEvent.TIMER,function(e:TimerEvent):void{
					...
					if(debugTxt != null)debugTxt.text = jsVar.toString();
					if(jsVar){
			...
			for each(var i:Object in params){
				if(i is Array){
					for each(var j:String in i){
						if(!checkURL(j)) return false;
					}
				}else{
					if(!checkURL(i as String)) return false;
				}
			}
			return true;
		}
		...
		public static function getParameters(stage:Stage,checkVar:String,back:Function,timeout:int = 0):void{
			...
			function loadp():void{
				...
				if(loadTimes > 0) loadTimes--;
				if(ckvar || loadTimes == 0){
					clearInterval(loadint);
					if(back != null) back(stage.loaderInfo.parameters);
				}
			}
		}
