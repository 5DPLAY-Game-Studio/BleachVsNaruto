...
//			_mc = mc;
		...
		public function getCurAction():String{
			if(_doingAirAction != null) return _doingAirAction;
			return _doingAction;
		}
		...
		public function idle(frame:String = "站立"):void{
			...
			_fighter.actionState = FighterActionState.NORNAL;
...
//				trace('!_isTouchFloor.fall');
		...
		public function setJump(action:String = "跳"):void{
			if(!_mc.checkFrame(action)) return;
			_action.jump = action;
		}
...
//			trace('setTouchFloor' ,action , breakAct);
		...
		public function setWankai():void{
			if(_mc.checkFrame("万解")) _action.waiKai = "万解";
			if(_mc.checkFrame("万解W")) _action.waiKaiW = "万解W";
			if(_mc.checkFrame("万解S")) _action.waiKaiS = "万解S";
		}
		...
		public function move(x:Number=0,y:Number=0):void{
			...
			if(_fighter.isInAir && x != 0) _action.airMove = false;
			x *= _fighter.direct;
...
//			var targetDisplay:DisplayObject = target ? target.getDisplay() : null;
			...
			if(x){
				if(x is Number){
					...
					if(x.target != undefined && target){
						...
						if(isNaN(Number(x.target))) mmc.x += Number(x.target);
					}
				}
			}
			...
			if(y){
				if(y is Number){
					...
					if(y.target != undefined && target){
						...
						if(isNaN(Number(y.target))) mmc.y += Number(y.target);
					}
				}
			}
		...
		public function getAttacker(name:String):FighterAttackerCtrler{
			...
			if(attacker) return attacker.getCtrler();
			return null;
		}
		...
		public function render():void{
			...
			if(_justDefenseFrame > 0) _justDefenseFrame--;
			
			_action.render();
			
			if(_moveTargetParam) renderMoveTarget();
			
			if(_actionCtrler) _actionCtrler.render();
...
//			renderAssist();
		...
		private function renderHurtFlying():void{
			...
			if(_hurtDownFrame > 0){
				if(!_actionLogic || !_actionLogic.enabled()) return;
...
//			if(_fighter.actionState != FighterActionState.NORNAL && _fighter.actionState != FighterActionState.DEFENCE_ING) return;
...
//			}
			...
			if(_actionLogic.specailSkill()) FighterEventDispatcher.dispatchEvent(_fighter,FighterEvent.DO_SPECIAL);
		...
		private function renderFloorAction():void{
			...
			if(_actionLogic.catch1()) doCatch(_action.catch1);
			if(_actionLogic.catch2()) doCatch(_action.catch2);
			
			if(_actionLogic.bishaSUPER()) doBisha(_action.bishaSUPER , _action.bishaSUPERQi , true);
			if(_actionLogic.bishaUP()) doBisha(_action.bishaUP , _action.bishaUPQi);
			if(_actionLogic.bisha()) doBisha(_action.bisha , _action.bishaQi);
			
			if(_actionLogic.skill2()) doSkill(_action.skill2);
			if(_actionLogic.skill1()) doSkill(_action.skill1);
			
			if(_actionLogic.zhao3()) doSkill(_action.zhao3);
			if(_actionLogic.zhao2()) doSkill(_action.zhao2);
			
			if(_actionLogic.attack()) doAttack(_action.attack);
			
			if(_actionLogic.zhao1()) doSkill(_action.zhao1);
			
			if(_actionLogic.defense()) doDefense();
			
			if(_actionLogic.dash()) doDash(_action.dash);
			
			if(_actionLogic.moveLEFT()) doMove(_action.moveLeft , -1);
			if(_actionLogic.moveRIGHT()) doMove(_action.moveRight , 1);
			
			if(_actionLogic.jump()) doJump(_action.jump);
			if(_actionLogic.jumpDown()) doJumpDown(_action.jumpDown);
			
			if(_action.isMoving) renderMoving();
			if(_action.isDefensing) renderDefense();
			
			if(_actionLogic.ghostStep()) doGhostStep();
			if(_actionLogic.ghostJump()) doGhostJump();
			
			//仅限连招使用
			if(FighterActionState.isAttacking(_fighter.actionState)){
				if(_actionLogic.attackAIR()) doAirAttack(_action.attackAIR);
				if(_actionLogic.skillAIR()) doAirSkill(_action.skillAIR);
				if(_actionLogic.bishaAIR()) doAirBisha(_action.bishaAIR , _action.bishaAIRQi);
			}
			
		}
		
		private function renderWanKaiCtrl():Boolean{
			if(!_actionLogic || !_actionLogic.enabled()) return false;
			
			if(_actionLogic.waiKai()) return checkDoWankai(_action.waiKai, "万解");
			if(_actionLogic.waiKaiW()) return checkDoWankai(_action.waiKaiW, "万解W");
			if(_actionLogic.waiKaiS()) return checkDoWankai(_action.waiKaiS, "万解S");
		...
		public function renderAnimate():void{
			...
			if(_mc) _mc.renderAnimate();
			
			if(_actionCtrler) _actionCtrler.renderAnimate();
			...
			if(_action){
				if(_action.isHurting) renderHurtAnimate();
				if(_action.isDefenseHiting) renderDefensHiting();
				
				if(_action.isJumping) renderJumpAnimate();
				
				if(_doingAction) _doActionFrame++;
				if(_action.isDefensing) renderDefenseAnimate();
			}
		...
		private function renderAirAction():void{
			...
			if(_actionLogic == null || !_actionLogic.enabled()) return;
			
			if(_actionLogic.attackAIR()) doAirAttack(_action.attackAIR);
			if(_actionLogic.skillAIR()) doAirSkill(_action.skillAIR);
			if(_actionLogic.bishaAIR()) doAirBisha(_action.bishaAIR , _action.bishaAIRQi);
			if(_actionLogic.jump()) doAirJump(_action.jump);
			if(_actionLogic.jumpQuick()) doAirJump(_action.jumpQuick);
			...
			if(_actionLogic.dash()) doDashAir(_action.dash);
			
			if(_actionLogic.airMove()) doAirMove();
			if(_actionLogic.ghostJump()) doGhostJump();
			if(_actionLogic.ghostJumpDown()) doGhostJumpDown();
		}
...
//			if(!selfDisplay) return;
			...
			if(_moveTargetParam.followMcName){
				...
				if(!isNaN(_moveTargetParam.x)) aimX = _moveTargetParam.x;
				if(!isNaN(_moveTargetParam.y)) aimY = _moveTargetParam.y;
...
//							(target as BaseGameSprite).isApplyG = false;
				...
				if(!isNaN(aimX)) target.x = aimX;
				if(!isNaN(aimY)) target.y = aimY;
			}
...
//			_mc.goFrame(action,false);
		...
		private function doAirMove():void{
			if(_actionCtrler.moveLEFT()) _fighter.move(-_fighter.speed);
			if(_actionCtrler.moveRIGHT()) _fighter.move(_fighter.speed);
		}
...
//			renderBeHitGap();
		...
		private function renderDefenseAnimate():void{
			if(_action.isDefenseHiting) return;
			if(_defenseFrameDelay-- > 0) return;
			...
			if(_actionCtrler.enabled() && _actionCtrler.defense()){
				...
				if(_defenseFrameDelay > -5) return;
				_action.isDefensing = false;
...
//			trace('setDefenseAction');
		...
		private function doDash(action:String):void{
			...
			if(!_fighter.hasEnergy(20,true)) return;
			_fighter.useEnergy(20);
			
			if(_actionCtrler.moveLEFT()) _fighter.direct = -1;
			if(_actionCtrler.moveRIGHT()) _fighter.direct = 1;
			doAction(action);
		...
		private function doDashAir(action:String):void{
			if(_action.jumpTimes < 1) return;
			if(!_fighter.hasEnergy(30,true)) return;
		...
		private function doJumpDown(acion:String):void{
			if(_fighter.isTouchBottom) return;
			_action.clear();
			_action.jumpTimes = 0;
...
//			}
			...
			if(!_fighter.isSuperSteelBody && (_fighter.energyOverLoad || hitvo.isBisha() || hitvo.isCatch())){
				doHurt(hitvo, hitRect);
				return;
			}
			...
			if(!_fighter.isSuperSteelBody){
				...
				if(hitvo.owner) hitx *= hitvo.owner.direct;
...
//			if(hitvo && hitRect) EffectCtrl.I.doHitEffect(hitvo , hitRect , _fighter);
			if(hitvo && hitRect) EffectCtrl.I.doSteelHitEffect(hitvo , hitRect , _fighter);
		}
		...
		private function doHurtAnimate(hitvo:HitVO , hitRect:Rectangle):void{
			...
			if(_fighter.isInAir){
				if(hity <= 0) hity -= GameConfig.HURT_Y_ADD_INAIR;
			}else{
				if(hity < 0){
			...
			if(hitvo.hurtType == 0){
				...
				_hurtHoldFrame = Math.round((hitvo.hurtTime / 1000) * GameConfig.FPS_ANIMATE) + GameConfig.HURT_FRAME_OFFSET;
				if(_hurtHoldFrame < GameConfig.HURT_GAP_FRAME) _hurtHoldFrame = GameConfig.HURT_GAP_FRAME;
		...
		private function renderHurtBreak():void{
			...
			if(!_actionCtrler.specailSkill()) return;
			
			if(!_fighter.hasEnergy(50)) return;
			if(_fighter.qi < 100) return;
...
//				trace("恢复被打");
		...
		private function hurtResume():void{
			...
			if(!_fighter.isInAir && !_isTouchFloor) _isTouchFloor = true;
			idle();
			FighterEventDispatcher.dispatchEvent(_fighter,FighterEvent.HURT_RESUME);
		}
...
//				trace('break def');
			...
			if(hitvo.hurtType == 0){
				_defenseHoldFrame = int((hitvo.hurtTime / 1000) * GameConfig.FPS_GAME / 5);
				if(_defenseHoldFrame < GameConfig.DEFENSE_HOLD_FRAME_MIN) _defenseHoldFrame = GameConfig.DEFENSE_HOLD_FRAME_MIN;
				if(_defenseHoldFrame > GameConfig.DEFENSE_HOLD_FRAME_MAX) _defenseHoldFrame = GameConfig.DEFENSE_HOLD_FRAME_MAX;
			}else{
			...
			if(hitvo && hitRect) EffectCtrl.I.doDefenseEffect(hitvo , hitRect , _fighter.defenseType);
...
//			renderBeHitGap();
			if(_defenseHoldFrame > 0){
				...
				if(_fighter.getVecX() == 0) _action.isDefenseHiting = false;
			}
		}
		...
		private function doHurtDownJump():void{
			if(_doingAction == '起身') return;
			if(_fighter.currentHurtDamage() > 240) return;
			if(!_fighter.hasEnergy(30)) return;
		...
		private function startGhostStep():Boolean{
			if(_fighter.qi < 60) return false;
			if(!_fighter.hasEnergy(80,true)) return false;
