...
//		private var _initAction:String = "开场";
		...
		public function getTargetState():int{
			...
			if(!target) return FighterActionState.NORMAL;
		...
		public function targetInRange(rx:Array = null , ry:Array = null):Boolean{
			...
			if(rx) XinRange = xDis >= rx[0] && xDis <= rx[1];
			if(ry) YinRange = yDis >= ry[0] && yDis <= ry[1];
...
//			if(fighterMc) fighterMc.renderAnimate();
			if(_fighterMcCtrl) _fighterMcCtrl.renderAnimate();
		}
...
//			if(fighterMc) fighterMc.render();
			if(_fighterMcCtrl) _fighterMcCtrl.render();
		}
		 ...
		 * @param id 攻击KEY
		 * @param obj 攻击数据
		 * sample : defineAction("tk" , {power:30 , hitType:1 , hitx:2 , hity:6 , hurtTime:300 , hurtType:0 , isBreakDef:false});
		 */
...
//			//缓存取出后，需要对重新计算位置
		...
		public function getCurrentRect(rect:Rectangle , cacheId:String = null):Rectangle{
			...
			if(_fighter.direct < 0) newRect.x -= rect.width;
...
//			var targetDisplay:DisplayObject = target.getDisplay();
			...
			if(setDirect) _fighter.direct = (_fighter.x < target.x) ? 1 : -1;
		}
