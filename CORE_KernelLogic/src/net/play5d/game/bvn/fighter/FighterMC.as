...
//			mcCtrler.setMc(this);
		...
		public function renderAnimate():void{
			if(_renderMainAnimate){
				if(_renderMainAnimateFrame > 0){
					if(--_renderMainAnimateFrame <= 0) _renderMainAnimate = false;
					_curMainFrameCount++;
				}
				try{
					_mc.nextFrame();
			...
			if(_hurtFlyState != 0) renderHurtFly();
			...
			for(i=0 ; i < length ; i++){
				...
				try{
					...
					if(mc){
						...
						if(mcName == "AImain" || mcName == "bdmn" || mcName.indexOf("atm") != -1) continue;
		...
		public function goFrame(name:String , isPlay:Boolean = true , playFrame:int = 0 , goFrameDelay:Object = null):void{
			...
			if(goFrameDelay && (goFrameDelay.name || goFrameDelay.call) && int(goFrameDelay.delay) > 0){
		...
		public function checkFrame(name:String):Boolean{
			if(_undefinedFrames.indexOf(name) != -1) return false;
...
//			}
		...
		public function getCurrentHitSprite():Array{
			...
			for(var i:int ; i < _mc.numChildren ; i++){
				...
				if(d && d.name.indexOf('atm') != -1) rt.push(d);
			}
			return rt;
		}
		...
		private function findBodyArea():void{
			if(!_bodyAreaCache) return;
			//判断是否已定义过
			if(_bodyAreaCache.areaFrameDefined(_mc.currentFrame)) return;
		...
		private function findHitArea():void{
			...
			//判断是否已定义过
			if(_hitAreaCache.areaFrameDefined(_mc.currentFrame)) return;
		...
		private function renderHurtFly():void{
			switch(_hurtFlyState){
				case 1: //击飞
					...
					if(_hurtYMin > _fighter.y) _hurtYMin = _fighter.y;
					break;
				case 2: //落地
					if(_curFrameCount < 2) return;
...
//						_fighter.setVelocity(0,0);
						...
						var vecy:Number = yDiff / 25 * (1+_hity*0.1);
...
//						trace('vecy',vecy);
		...
		private function initHitRange():void{
			...
			for each(var i:String in FighterHitRange.getALL()){
				...
				if(d) _hitRangeObj[i] = d.getBounds(_fighterDisplay);
			}
			hrmc.gotoAndStop(1);
			hrmc.visible = false;
		}
