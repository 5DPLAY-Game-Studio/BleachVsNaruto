...
//		private var _lastPos:Point = new Point();
		...
		{
			_mainMc = mainmc;
			if(_mainMc) _area = _mainMc.getBounds(_mainMc);
		}
...
//			_lastPos = new Point(x,y);
		...
		public function move(x:Number=0 , y:Number = 0):void{
			if(x != 0) _x += x * _speedPlus;
			if(y != 0) _y += y * _speedPlus;
		}
...
//		}
		...
		private function renderVelocity():void{
			...
			if(_velocity.x != 0){
				resultX += _velocity.x;
				if(_damping.x > 0) _velocity.x = KyoUtils.num_wake(_velocity.x,_damping.x * _dampingRate);
			}
			if(_velocity.y != 0){
				resultY += _velocity.y;
				if(_damping.y > 0) _velocity.y = KyoUtils.num_wake(_velocity.y,_damping.y * _dampingRate);
			}
			if(_velocity2.x != 0){
				resultX += _velocity2.x;
				if(_damping2.x > 0) _velocity2.x = KyoUtils.num_wake(_velocity2.x,_damping2.x * _dampingRate);
			}
			if(_velocity2.y != 0){
				resultY += _velocity2.y;
				if(_damping2.y > 0) _velocity2.y = KyoUtils.num_wake(_velocity2.y,_damping2.y * _dampingRate);
			}
...
//			_y += _g * _speedPlus;
		...
		private function renderAnimateFrameOut():void{
			if(_frameAnimateFuncs.length < 1) return;
		...
		private function renderFrameOut():void{
			if(_frameFuncs.length < 1) return;
