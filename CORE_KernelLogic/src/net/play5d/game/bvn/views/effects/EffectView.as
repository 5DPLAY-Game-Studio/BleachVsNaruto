...
package net.play5d.game.bvn.views.effects
{
	...
	public class EffectView
	{
		...
		public function start(x:Number = 0 , y:Number = 0 , direct:int = 1, playSound:Boolean = true):void{
			...
			if(_data.randRotate) randRotate();
			if(playSound && _data.sound) SoundCtrl.I.playEffectSound(_data.sound);
		...
		public function gotoAndPlay(frame:Object):void{
			if(frame is int) _curFrame = int(frame);
			if(frame is String){
		...
		public function addRemoveBack(func:Function):void{
			_onRemoveFuncs ||= [];
			if(_onRemoveFuncs.indexOf(func) != -1) return;
