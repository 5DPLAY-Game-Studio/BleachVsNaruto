...
//			gameRunData.reset();
		...
		public function renderAnimate():void{
			...
			if(!GameCtrl.I.actionEnable) return;
			...
			if(_changeFighterGap > 0) _changeFighterGap--;
		}
		...
		private function renderResumeFighters():void{
			if(!GameCtrl.I.gameRunData || !GameCtrl.I.gameRunData.p1FighterGroup) return;

			if(_resumeGap-- > 0) return;

			var fighters:Vector.<FighterMain> = GameCtrl.I.gameRunData.p1FighterGroup.getHoldFighters();
			if(!fighters || fighters.length < 1) return;
			...
			for each(var f:FighterMain in fighters){
				...
				if(f.qi < f.qiMax){
					f.qi += f.qiMax * 0.02;
					if(f.qi > f.qiMax) f.qi = f.qiMax;
				}
				if(f.energy < f.energyMax){
					f.energy += f.energyMax * 0.08;
					if(f.energy > f.energyMax) f.energy = f.energyMax;
				}
			}
		...
		private function renderAddEnemy():void{
			if(_enemyCreators.length < 1) return;
...
//			if(_addEnemyGap++ < 15) return;
		...
		private function renderRepeat(data:MosouWaveRepeatVO):void{
			if(!data.enemies || data.enemies.length < 1) return;
...
//				trace('==== renderRepeat '+result.length+'   ===============================');
		...
		public function getWavePercent():Number{
			if(currentWave >= waveCount) return 1;
			if(_renderTimerMax < 1) return 0;
		...
		public function addEnemyFighter(fighter:FighterMain):void{
			...
			if(!fighter.initlized()){
				...
				if(fighter.mosouEnemyData && fighter.mosouEnemyData.isBoss){
					...
					(ctrl as EnemyBossAICtrl).AILevel = 2;
					(ctrl as EnemyBossAICtrl).fighter = fighter;
				}else{
					ctrl = new EnemyFighterAICtrl();
					(ctrl as EnemyFighterAICtrl).fighter = fighter;
				}
			...
			var disX:Number = 100 + (Math.random() * 100);
			var disY:Number = 50 + (Math.random() * 50);

			fighter.x = Math.random() > 0.5 ? (p1.x + disX) : (p1.x - disX);
			fighter.y = Math.random() > 0.5 ? (p1.y + disY) : (p1.y - disY);
		...
		private function getEnemyByData(mosouData:MosouEnemyVO):FighterMain{
			var filtered:Vector.<FighterMain> = _stageEnemies.filter(function(f:FighterMain, i:int, a:Vector.<FighterMain>):Boolean{
		...
		private function checkNextOrWin(onBossDie:Boolean = false):void{
			...
			if(_runningWaves.length > 0){
				if(aliveEnemies.length < 1){
					if(_renderTimer > 2 * GameConfig.FPS_ANIMATE) _renderTimer = 2 * GameConfig.FPS_ANIMATE;
				}
				return;
			}
...
//					KyoTimeout.setTimeout(missionComplete, 2 * GameConfig.FPS_GAME);
		...
		private function getAliveEnemies():Vector.<FighterMain>{
			return _stageEnemies.filter(function(f:FighterMain, i:int, a:Vector.<FighterMain>):Boolean{
				if(f.mosouEnemyData && f.mosouEnemyData.repeat) return false;
				return f.isAlive;
			});
		}

		public function changeFighter(from:FighterMain, to:FighterMain):void{
			if(_changeFighterGap > 0) return;
		...
		public function updateEnemy(v:FighterMain):void{
			if(_enemyBarCtrl) _enemyBarCtrl.updateEnemyBar(v);
		}
		...
		public function updateCamera():void{
			...
			var focuses:Array = [];
			if(p1) focuses.push(p1.getDisplay());
			if(p2) focuses.push(p2.getDisplay());
		...
		private function renderBossIn():void{
			...
			if(!_bossInAnimate && (_introBoss.actionState != FighterActionState.KAI_CHANG) ){
				_introBoss = null;
...
internal class EnemyCreator{
	...
	public function getNextEnemy():MosouEnemyVO{
		if(_addEnemies.length < 1) return null;
