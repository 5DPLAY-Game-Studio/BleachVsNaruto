...
//				}
		...
		public function renderInput():void{
			...
			if(!_inputers || _inputers.length < 1) return;
			...
			for(i=0 ; i < l ; i++){
				...
				p.up() && (_inputData |= _upK);
				p.down() && (_inputData |= _downK);
				p.left() && (_inputData |= _leftK);
				p.right() && (_inputData |= _rightK);
				
				p.attack() && (_inputData |= _attackK);
				p.jump() && (_inputData |= _jumpK);
				p.dash() && (_inputData |= _dashK);
				
				p.skill() && (_inputData |= _skillK);
				p.superSkill() && (_inputData |= _bishaK);
				p.special() && (_inputData |= _specialK);
