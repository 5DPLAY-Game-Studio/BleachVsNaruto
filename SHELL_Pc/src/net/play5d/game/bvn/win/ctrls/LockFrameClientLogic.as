...
package net.play5d.game.bvn.win.ctrls
{
	...
	public class LockFrameClientLogic
	{
		...
		public function receiveUpdate(msgArr:ByteArray):Boolean{
			if(!msgArr) return false;

			msgArr.position = 0;

			var type:int = msgArr.readByte();

			if(type != MsgType.INPUT_UPDATE) return false;
		...
		public function receiveSyncUpdate(msgArr:ByteArray):Boolean{
			...
			if(type != MsgType.INPUT_SYNC) return false;
...
//			trace('receive sync update', 'frame='+frame, 'round='+round, 'time='+time, 'p1hp='+p1hp, 'p1qi='+p1qi, 'p1x='+p1x, 'p1y='+p1y, ' || ' ,'p2hp='+p2hp, 'p2qi='+p2qi, 'p2x='+p2x, 'p2y='+p2y);
			_serverFrame = frame;
...
//			InputManager.I.socket_input_p2.renderInput();
			...
			if(_lastSendK == k && !_sendAnyWay) return;
